...
class Objet
{
    ...
    {
        $stmt = $pdo->prepare('INSERT INTO objets (name, description, owner_id) VALUES (:name, :description, :owner_id)');
        $stmt->execute([
            'name' => $this->name,
    ...
    {
        $stmt = $pdo->prepare('UPDATE objets SET name = :name, description = :description WHERE id = :id');
        $stmt->execute([
            'name' => $this->name,
            'description' => $this->description,
            'id' => $this->id
