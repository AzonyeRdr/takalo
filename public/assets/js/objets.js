...
$(document).ready(function () {
  ...
  function rechercherObjets() {
    ...
    $.ajax({
      ...
      error: function () {
        $("#objetsContainer").html(
          '<div class="col-12 text-center"><p>Erreur lors de la recherche.</p></div>',
        );
      },
    });
  }
  ...
  function afficherResultats(objets) {
    ...
    if (objets.length === 0) {
      container.html(
        '<div class="col-12 text-center"><p>Aucun objet trouvé.</p></div>',
      );
      return;
    }

    objets.forEach(function (obj) {
      var prix = obj.prix_estime
        ? parseFloat(obj.prix_estime).toFixed(2) + " Ar"
        : "À négocier";
      var card =
        ...
        "<h4>" +
        escapeHtml(obj.titre) +
        "</h4>" +
        "<span>" +
        prix +
        "</span>" +
        "<p><small>Par " +
        escapeHtml(obj.proprietaire) +
        " | " +
  ...
  $("#photosInput").on("change", function () {
    ...
    if (!files || files.length === 0) {
      $("#photosPreviewContainer").hide();
      return;
    }
    // show preview container when files selected
    $("#photosPreviewContainer").show();
      ...
      (function (index) {
        ...
        reader.onload = function (e) {
          ...
          var html =
            ...
            '<div class="mt-1">' +
            '<label><input type="radio" name="photo_principale" value="' +
            index +
            '" ' +
            checked +
            "> Principale</label>" +
